...
// Plain HTTP auth: Firebase sign-up → POST /api/auth/firebase returns {user} (no JWT);
// subsequent calls pass userId in body/query — no Authorization header.
...
async function main() {
  ...
  console.log("\n=== 2. Backend Firebase upsert (POST /api/auth/firebase) ===");
  ...
  assert("Backend user returned (no JWT)", authRes.ok && !!authJson.user?.id && !authJson.token, JSON.stringify(authJson).slice(0, 300));
  assert("User displayName stored correctly", authJson.user?.displayName === "E2E Test User", authJson.user?.displayName);
  assert("passwordHash not exposed", !authJson.user?.passwordHash, "passwordHash should never be in response");
  const userId = authJson.user.id;
  const headers = { "Content-Type": "application/json" };

  console.log("\n=== 3. GET /api/auth/me?userId= ===");
  const meRes = await fetch(`${NODE_API}/api/auth/me?userId=${encodeURIComponent(userId)}`, { headers });
  ...
  const sessionRes = await fetch(`${NODE_API}/api/sessions`, {
    method: "POST", headers, body: JSON.stringify({ userId, modality: "text" }),
  });
  ...
  const chatRes = await fetch(`${NODE_API}/api/chat`, {
    method: "POST", headers,
    body: JSON.stringify({ userId, sessionId, content: "Hi Nea, I'm testing the app, just say hello back.", modality: "text" }),
  });
  ...
  if (audioBytes) {
    ...
    const sttRes = await fetch(`${NODE_API}/api/voice/stt`, { method: "POST", body: form });
  ...
  const checkinRes = await fetch(`${NODE_API}/api/memory/checkin`, {
    method: "POST", headers, body: JSON.stringify({ userId, mood: "okay" }),
  });
  const checkinJson = await checkinRes.json();
  assert("Check-in saved", checkinRes.ok && checkinJson.checkIn?.mood === "okay", JSON.stringify(checkinJson).slice(0, 150));

  const weekRes = await fetch(`${NODE_API}/api/memory/checkin/week?userId=${encodeURIComponent(userId)}`, { headers });
  ...
  const crisisSessionRes = await fetch(`${NODE_API}/api/sessions`, {
    method: "POST", headers, body: JSON.stringify({ userId, modality: "text" }),
  });
  ...
  const crisisRes = await fetch(`${NODE_API}/api/chat`, {
    ...
    body: JSON.stringify({
      userId,
      sessionId: crisisSessionJson.session.id,
  ...
  const consolidateRes = await fetch(`${NODE_API}/api/memory/consolidate`, {
